...
class TestScoreboardTextManager(unittest.TestCase):
    ...
    def test_gender_matchup_color_wmp(self):
        """Test that WMP matchup gets orange color."""
        self.text_manager.set_text("gender_matchup", "WMP")
        label = self.text_manager.text_elements["gender_matchup"]["label"]
        self.assertEqual(label.color, 0xFFA500)  # WMP_GENDER_MATCHUP_COLOR

    def test_gender_matchup_color_mmp(self):
        """Test that MMP matchup gets green color."""
        self.text_manager.set_text("gender_matchup", "MMP")
        label = self.text_manager.text_elements["gender_matchup"]["label"]
        self.assertEqual(label.color, 0x00AA00)  # MMP_GENDER_MATCHUP_COLOR

    def test_gender_matchup_counter_color(self):
        """Test that gender matchup counter gets correct color."""
        self.text_manager.set_text("gender_matchup_counter", "MMP")
        label = self.text_manager.text_elements["gender_matchup_counter"]["label"]
        self.assertEqual(label.color, 0x00AA00)  # MMP_GENDER_MATCHUP_COLOR
